...
class GeminiService
{
    ...
    /**
     * Перевіряє, чи є введений текст назвою препарату.
     * Повертає true, якщо це назва препарату; false, якщо це симптоми або опис.
     */
    public function isMedicineName(string $input): bool
    {
        $prompt = <<<PROMPT
Ти — медичний експерт. Визнач, чи є наступний текст назвою лікарського препарату (торговою назвою чи МНН), чи це опис симптомів/показань.

Текст для аналізу: "{$input}"

Поверни JSON:
{
  "is_medicine_name": true або false,
  "reason": "коротке пояснення українською (1-2 речення)"
}

Критерії:
- is_medicine_name = true: конкретна назва препарату (наприклад: "Нурофен", "Ібупрофен", "Парацетамол", "Аспірин", "Німесил")
- is_medicine_name = false: опис симптомів, показань, скарг пацієнта (наприклад: "головний біль", "температура", "біль у животі", "кашель і нежить")
PROMPT;

        $data = $this->askJson($prompt);

        return (bool) ($data['is_medicine_name'] ?? false);
    }
}
